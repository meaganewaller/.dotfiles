#!/usr/bin/env bash
# governance.sh - Unified CLI for policy provenance governance.
#
# Usage: governance.sh [MODE] [OPTIONS]
#
# Modes:
...
GOVERNANCE_ROOT="${GOVERNANCE_ROOT:-$DOTFILES_ROOT/home/.claude/governance}"
...
usage() {
  cat <<EOF
${BOLD}governance${NC} - Policy provenance governance CLI

${BOLD}USAGE${NC}
    governance.sh [MODE] [OPTIONS]
...
${BOLD}EXAMPLES${NC}
    ...
    governance.sh --trace commit      # Trace commit cue
    governance.sh --control ENG-COMMIT-001
    governance.sh --gaps              # Find missing provenance
    governance.sh --lint              # Validate all provenance
    governance.sh --matrix --json     # Export for auditors
EOF
}
...
generate_manifest() {
  ...
  if [[ -d "$claude_home/cues" ]]; then
    cue_roots="$claude_home/cues"
  fi

  # Also include dotfiles cues
  local dotfiles_cues="$DOTFILES_ROOT/home/.claude/cues"
