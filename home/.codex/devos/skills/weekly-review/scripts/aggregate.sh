...
case "$MODE" in
  combined) DEFAULT_EVENT_STREAMS=("${DEFAULT_COMBINED_STREAMS[@]}") ;;
  codex)    DEFAULT_EVENT_STREAMS=("${DEFAULT_CODEX_STREAMS[@]}") ;;
  claude)   DEFAULT_EVENT_STREAMS=("${DEFAULT_CLAUDE_STREAMS[@]}") ;;
  *)
    echo "Error: Invalid WEEKLY_REVIEW_MODE=$MODE (use codex|claude|combined)" >&2
    exit 1
    ;;
esac

STREAM_INPUT="${CODEX_EVENT_STREAMS:-}"
if [[ -n "${CODEX_EVENT_STREAM:-}" ]]; then
  STREAM_INPUT="${CODEX_EVENT_STREAM}:${STREAM_INPUT}"
fi

declare -a CANDIDATE_STREAMS=()
if [[ -n "$STREAM_INPUT" ]]; then
  STREAM_INPUT="${STREAM_INPUT//,/:}"
  IFS=':' read -r -a CANDIDATE_STREAMS <<< "$STREAM_INPUT"
else
  CANDIDATE_STREAMS=("${DEFAULT_EVENT_STREAMS[@]}")
fi

declare -a EVENT_STREAMS=()
STREAM_SEEN=":"
for stream in "${CANDIDATE_STREAMS[@]-}"; do
  [[ -f "$stream" ]] || continue
  case "$STREAM_SEEN" in
    *":$stream:"*) continue ;;
  esac
  EVENT_STREAMS+=("$stream")
  STREAM_SEEN="${STREAM_SEEN}${stream}:"
done

if [[ "${#EVENT_STREAMS[@]}" -eq 0 ]]; then
  echo "Error: Missing event streams. Checked defaults:" >&2
  for stream in "${DEFAULT_EVENT_STREAMS[@]}"; do
    echo "  - $stream" >&2
  done
  echo "Override with CODEX_EVENT_STREAMS=\"/path/one.jsonl:/path/two.jsonl\"" >&2
  exit 1
fi

EVENT_STREAMS_ARG="$(IFS=:; echo "${EVENT_STREAMS[*]}")"
...
python3 - <<PY "$EVENT_STREAMS_ARG" "$PROJECTS_DIR" "$SUMMARY_JSON" "$WEEK_START" "$WEEK_END"
import json, sys, datetime as dt
from collections import Counter, defaultdict
from pathlib import Path

stream_paths = [p for p in sys.argv[1].split(":") if p]
            ...
            if t >= since:
                ...
                if event_type != "decision_tradeoff":
                    if not session_id or session_id == "unknown" or session_id.startswith("test"):
                        continue
                # De-dupe by event identity across streams.
                key = json.dumps(
                    {
