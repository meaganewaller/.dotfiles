...
functions.toint = function(float)
	return float | 0
end
...
functions.unpack = table.unpack or unpack

function functions.utf8(decimal)
	if decimal < 128 then
...
function functions.send_utf8(decimal)
	return act.SendString(functions.utf8(decimal))
end

function functions.send_escape(sequence)
