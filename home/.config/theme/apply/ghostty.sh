#!/usr/bin/env bash
set -euo pipefail

THEME_DIR="${HOME}/.config/theme"
CURRENT="${THEME_DIR}/current.json"
# Resolve symlinks: macOS sed -i cannot edit in place through a symlink
GHOSTTY_CONFIG="$(python3 -c "import os,sys; print(os.path.realpath(sys.argv[1]))" "${HOME}/.config/ghostty/config")"

[ -f "$CURRENT" ] || exit 0
[ -f "$GHOSTTY_CONFIG" ] || exit 0

theme_name=$(python3 -c 'import json; print(json.load(open("'"$CURRENT"'")).get("ghostty",{}).get("theme",""))' 2>/dev/null || true)
[ -n "$theme_name" ] || exit 0

# Build the theme line
new_line="theme = ${theme_name}"

# Replace the theme line in ghostty config
if grep -q '^theme = ' "$GHOSTTY_CONFIG"; then
  sed -i '' "s|^theme = .*|${new_line}|" "$GHOSTTY_CONFIG"
fi

## HACKY 
osascript -e 'tell application "System Events" to tell process "ghostty" to keystroke "," using {command down, shift down}'